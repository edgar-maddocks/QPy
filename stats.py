...
def annualize_rets(returns, ppy, start_date = None, end_date = None):
    if start_date is None and end_date is None:
        ...
        annualized = ((1 + returns[:end_date]).prod()) ** (ppy/returns[:end_date].shape[0]) - 1
    elif start_date is not None and end_date is None:
        if isinstance(start_date, str) is False:
            raise Exception("start date must be a string")
        annualized = ((1 + returns[start_date:]).prod()) ** (ppy/returns[start_date:].shape[0]) - 1
    elif isinstance(start_date, str) and isinstance(end_date, str):
        annualized = ((1 + returns[start_date:end_date]).prod()) ** (12/returns[start_date:end_date].shape[0]) - 1
    return annualized

def annualize_vol(returns, ppy, start_date = None, end_date = None):
    if start_date is None and end_date is None:
        annualized = returns.std()*(ppy**.5)
    elif start_date is None and end_date is not None:
        if isinstance(end_date, str) is False:
            raise Exception("end date must be a string")
        annualized = returns[:end_date].std()*(ppy**.5)
    elif start_date is not None and end_date is None:
        if isinstance(start_date, str) is False:
            raise Exception("start date must be a string")
        annualized = returns[start_date:].std()*(ppy**.5) * 100
    elif isinstance(start_date, str) and isinstance(end_date, str):
        annualized = returns[start_date:end_date].std()*(12**.5)
    return annualized
...
def max_dd(returns, start_date = None, end_date = None):
    ...
    return max_drawdown, pd.DataFrame({"Wealth": wealth_index, 
