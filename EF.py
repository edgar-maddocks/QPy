...
def minimize_vol(target_return, returns, cov_matrix, weight_bounds=(0.0, 1.0)):
...
def maximize_sr(returns, cov_matrix, risk_free_rate=0, weight_bounds=(0.0, 1.0)):
    n_assets = returns.shape[0]
    init_guess = np.repeat(1 / n_assets, n_assets)
    bounds = (weight_bounds,) * n_assets
...
def plot_ef(
    ...
    weights = optimal_weights(n_points=n_points, returns=ann_rets, cov=cov_mat)
    ...
    ef = pd.DataFrame(
        {
            "V": pt_vol,
            "R": pt_rets,
            "SR": ((np.array(pt_rets) - risk_free_rate) / np.array(pt_vol)),
        }
    )
    if verbose:
    ...
    if plot_max_SR:
        ...
        if draw_cml:
            ...
            axs.plot(
                cml_x, cml_y, c="red", label="Capital Market Line", linestyle="dashed"
            )
            axs.legend()
    if plot_ew:
...
response = ef_data(data["2018":][l], 12, risk_free_rate=0.1)
print(response["GMV"]["weights"])
print(gmv(data["2018":][l].cov()))

# Assume two different investors invested in the GMV and MSR portfolios at the
# start of 2018 using the weights we just computed.
# Compute the annualized volatility of these two portfolios
# over the next 12 months of 2018? (Hint: Use the portfolio_vol code we developed in
# the lab and use ind[“2018”][l].cov() to compute the covariance matrix for 2018,
# assuming that the variable ind holds the industry returns and the variable l holds the
# list of industry portfolios you are willing to hold. Don’t forget to annualize the volatility)
