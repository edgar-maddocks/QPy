...
class DQNAgent:
    ...
    def update_weights(self, batch_size):
        ...
        states = torch.stack(([torch.Tensor([x[0]]) for x in replay_batch])).to(
            self.device
        )
        rewards = torch.stack(([torch.Tensor([x[1]]) for x in replay_batch])).to(
            self.device
        )
        done_mask = torch.stack(
            ([torch.Tensor([1]) if x[2] else torch.Tensor([0]) for x in replay_batch])
        ).to(self.device)
        actions = [x[3] for x in replay_batch]
        next_states = torch.stack(([torch.Tensor([x[4]]) for x in replay_batch])).to(
            self.device
        )
        ...
        oh_actions = one_hot(torch.LongTensor(actions), self.env.action_space.n).to(
            self.device
        )
        ...
        loss = (
            (
                rewards
                + (self.gamma * (done_mask * next_qvals))
                - torch.sum(qvals * oh_actions, -1)
            )
            ** 2
        ).mean()
    ...
    def train(
        ...
        if use_wandb:
            wandb.login()
            log = wandb.init(project="DQN", config={"epsilon": self.epsilon})
        if return_data:
            loss_data = []
            reward_data = []
            epsilon_data = []
        ...
        for episode in range(episodes):
            ...
            while not done:
                ...
                if (
                    len(self.replay.memory) > min_mem_size
                    and steps_before_train < steps_last_train
                ):
                    ...
                    print(
                        f"Episode : {episode}, Steps taken : {steps}, Loss: {loss.detach().item()}, Reward: {sum(running_reward)}"
                    )
                    steps_last_train = 0
            ...
            if use_wandb:
                if len(running_loss) > 0:
                    ...
                    wandb.log({"reward": sum(running_reward), "epsilon": self.epsilon})
            if return_data:
                if len(running_loss) > 0:
